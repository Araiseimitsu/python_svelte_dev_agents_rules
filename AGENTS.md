...
- [ ] ループ開発の提案条件（30行以上・複数ファイル/レイヤー・業務ロジック/API/DB・新規モジュール）に該当するか判定したか（§18）
...
- 条件に該当するタスクでは、計画フェーズでマルチエージェント活用・ループ開発を必ず提案する（§18）。
...
プロセスの詳細（提案フォーマット・ループ手順・終了条件）は `.docs/workflow.md` §6 に従う。

### 提案義務

計画フェーズで、タスクが以下の **いずれかに該当する場合、マルチエージェント活用またはループ開発の実施を計画に含めて必ず提案する**。

- 30行以上の変更が見込まれる
- 複数ファイル・複数レイヤーに跨る変更である
- 業務ロジック・判定処理・API・DB に関わる変更である
- 新規モジュールを追加する

### 承認ゲート

- ループ開発は **ユーザーが承認した場合のみ** 実行する。承認されなければ通常フロー（理解→計画→実装→検証→報告）で進める。
- 一度の承認は当該タスク限りとする。次のタスクでは改めて提案する。
...
- ユーザー承認なしでループ開発（自律レビューループ）を開始すること
